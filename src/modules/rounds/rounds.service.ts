...
export async function getStewardHistory(
  ...
  try {
    ...
    for (let i = 0; i < relevantKeys.length; i++) {
      ...
      const type: "nightly" | "debrief" = key.includes(":nightly:")
        ? "nightly"
        : "debrief";
...
function buildNightlySummary(data: any): string {
  ...
  if (s.reposWithChanges > 0)
    parts.push(`${s.reposWithChanges} repo(s) with uncommitted changes`);
  if (s.reposWithStaleBranches > 0)
    parts.push(`${s.reposWithStaleBranches} stale branch(es)`);
  if (s.unhealthyServices > 0)
    parts.push(`${s.unhealthyServices} unhealthy service(s)`);
...
function buildDebriefSummary(data: any): string {
  const parts: string[] = [];
  if (data.overnightCompleted > 0)
    parts.push(`${data.overnightCompleted} completed`);
  if (data.overnightFailed > 0) parts.push(`${data.overnightFailed} failed`);
  if (data.overnightCostUsd > 0)
    parts.push(`$${(data.overnightCostUsd as number).toFixed(4)} cost`);
...
export async function getQueueState(redisUrl?: string): Promise<QueueState> {
  ...
  try {
    ...
    const [pendingRaw, runningHash, failedRaw, completedRaw] =
      await Promise.all([
    ...
    state.running = Object.values(runningHash ?? {})
      .map((r) => safeJson<QueueTask>(r))
      .filter(Boolean) as QueueTask[];
    state.failed = failedRaw
      .map((r) => safeJson<QueueTask>(r))
      .filter(Boolean) as QueueTask[];
    state.completed = completedRaw
      .map((r) => safeJson<QueueTask>(r))
      .filter(Boolean) as QueueTask[];
...
export async function getProjects(
  ...
  try {
    ...
    const raws = await Promise.all(
      allIds.map((id) => redis.get(`ainulindale:project:${id}`)),
    );
    ...
    for (const raw of raws) {
      ...
      const subtasks: AinulindaleProject["subtasks"] = (
        data.subtasks ?? []
      ).map((st: any) => ({
      ...
      const progressPct =
        total > 0 ? Math.round((completedSubtasks / total) * 100) : 0;
