...
<body>
    <div class="container">
        <div class="header">
            <h1>🏠 Mithrandir API Dashboard</h1>
            <p>Your homelab management center</p>
        </div>

        <div class="dashboard-grid">
            <div class="card">
                <h3>🎙️ Transcription Projects</h3>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🎙️ Transcription Project Dashboard</h1>
    ...
    <script>
        ...
        function renderProjects(projects) {
            ...
            html += '<h3>📊 Summary: ' + totalFiles + ' Total Files</h3>';
            html += '<div class="summary-badges">';
            if (totalCompleted > 0) {
                html += '<span class="stat stat-completed"><span>✅</span>' + totalCompleted + ' completed</span>';
            }
            if (totalProcessing > 0) {
                html += '<span class="stat stat-processing"><span>🔄</span>' + totalProcessing + ' processing</span>';
            }
            if (totalPending > 0) {
                html += '<span class="stat stat-pending"><span>⏳</span>' + totalPending + ' pending</span>';
            }
            if (totalFailed > 0) {
                html += '<span class="stat stat-failed"><span>❌</span>' + totalFailed + ' failed</span>';
            }
            html += '</div>';
            html += '</div>';
        ...
        function renderAllFilesTable(projects) {
            ...
            html += '<th style="width: 100px;">⏱️ Elapsed</th>';
            html += '<th style="width: 100px;">🔄 Processing</th>';
        ...
        function renderFileRowWithProject(file) {
            var statusIcons = { completed: '✅', processing: '🔄', pending: '⏳', failed: '❌' };
            var formatDate = function(dateStr) { return dateStr ? new Date(dateStr).toLocaleDateString() : '—'; };
            var formatFileSize = function(bytes) { return bytes ? (bytes / (1024 * 1024)).toFixed(1) + ' MB' : '—'; };
            ...
            // Elapsed time column
            html += '<td>';
            html += '<span class="file-meta">' + (file.elapsedFormatted || '—') + '</span>';
            html += '</td>';

            // Processing time column
            html += '<td>';
            html += '<span class="file-meta">' + (file.processingFormatted || '—') + '</span>';
            html += '</td>';
        ...
        function updateFilterSummary(visible, total) {
            ...
            if (summaryStats) {
                if (visible === total) {
                    summaryStats.textContent = '📊 Summary: ' + total + ' Total Files';
                } else {
                    summaryStats.textContent = '📊 Filtered: ' + visible + ' of ' + total + ' Files';
                }
            }
        }
        ...
        function sortProjects() {
            ...
            sortBtn.innerHTML = '<span>↕️</span> Sort ' + (sortAscending ? 'Z-A' : 'A-Z');
